...
namespace ChatGPT.Net
{
    ...
    public class ChatGpt
    {
        ...
        private /*async*/ IEnumerable<string> StreamCompletion(Stream stream)
        {
            ...
            {
                Task<string> lineTask = /*await*/ reader.ReadLineAsync();
        ...
        {
            var conversation = GetConversation(conversationId);
            conversation.Messages = conversation.Messages.Where(
                x => x.Role != "system").ToList();
        ...
        public async Task<string> AskStream(Action<string> callback, 
            string prompt, string? conversationId = null)
        {
            var conversation = GetConversation(conversationId);
            ...
            try
            {
                result = reply.Choices.FirstOrDefault()?.Message.Content ?? "";
            }
            catch (Exception ex) 
            {
                Debug.WriteLine("[ex] AskStream : " + ex.Message); 
            }
            return result;
        }

        public async Task<ChatGptResponse> SendMessage(ChatGptRequest requestBody, 
            Action<ChatGptStreamChunkResponse>? callback = null)
        {
            ...
            if (((int)sc) != 200)
            {
                Debug.WriteLine("[ex] :" + "sc=" + sc.ToString());
                return default;
            }
            ...
            if (requestBody.Stream)
            {
                ...
                if (contentType != "text/event-stream")
                {
                    ...
                    Debug.WriteLine("[ex] Content Type exception : " + "Unknown error");
                    return default;
                }
                ...
                foreach (var data in StreamCompletion(stream))
                {
                    ...
                    reply = JsonConvert.DeserializeObject<ChatGptStreamChunkResponse>(
                        jsonString);
            ...
            //await 
            foreach (var data in StreamCompletion(contentStream))
            {
                var jsonString = data.Replace("data: ", "");
                ...
                content = JsonConvert.DeserializeObject<ChatGptResponse>(
                    jsonString);

                if (content is null) continue;
                //concatMessages += content.Choices.FirstOrDefault()?.Delta.Content;
